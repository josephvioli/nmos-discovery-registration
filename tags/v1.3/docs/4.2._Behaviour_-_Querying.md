...
In order to connect to a websocket, a client must first request a subscription of a particular type and with particular query parameters by performing a POST as defined in the [Query API](../html-APIs/QueryAPI.html) subscriptions documentation. Use of GET requests to find existing suitable websocket connections is strongly discouraged.
...
The timestamps used in these messages relate to the [NMOS Content Model](https://amwa-tv.github.io/nmos-content-model). In this use case the three timestamps are permitted to be identical.
...
A schema for the Grain metadata described below is available [here](../html-APIs/schemas/with-refs/queryapi-subscriptions-websocket.html).
